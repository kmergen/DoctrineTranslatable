...
/** @var \Kmergen\DoctrineTranslatable\Contract\Entity\TranslatableInterface $category */
...
This library provides an interface `Kmergen\DoctrineTranslatable\Contract\Provider\LocaleProviderInterface` that [by default returns the current locale using Symfony `RequestStack` or `TranslatorInterface`](https://github.com/KnpLabs/DoctrineBehaviors/blob/master/src/Provider/LocaleProvider.php).
